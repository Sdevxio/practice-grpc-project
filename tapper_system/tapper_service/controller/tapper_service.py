...
class TapperService:
    ...
    def send_command(self, command: str, params: Optional[dict] = None) -> any:
        """
        Send a raw commands directly to the tapper device.
        ...
        :raise TapperError: If not connected or commands execution fails.
        """
        self._ensure_connected()

        try:
            self.logger.debug(f"Sending commands '{command}' to station '{self.station_id}'")
            ...
            self.logger.error(f"Failed to send commands '{command}' to station '{self.station_id}': {e}")
            raise
