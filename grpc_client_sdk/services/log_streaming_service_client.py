...
class LogsMonitoringServiceClient:
    ...
    def __init__(self, client_name: str = "root", logger: Optional[object] = None):
        ...
        self.logger = logger or get_logger(f"service.log_streaming.{client_name}")
        self.stub = None
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            self.connected = True
            self.logger.debug("Logs Monitor Streaming Service Client - connected")
    ...
    def stream_entries_for_tap_correlation(self,
        ...
        try:
            # Use the streaming service with tap correlation
    ...
    def stream_log_entries(self,
        ...
        """Start streaming log entries using the gRPC service."""
        if not self.connected or not self.stub:
                ...
                try:
                    ...
                    for response in grpc_call:
                        ...
                        if self.stop_flags.get(stream_id, False):
                            self.logger.debug(f"Stream {stream_id} stopping due to stop flag")
                            break
                    ...
                    self.logger.debug(f"Stream {stream_id} ended: {stream_error}")
