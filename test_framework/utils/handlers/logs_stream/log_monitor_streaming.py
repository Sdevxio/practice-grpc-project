...
class LogMonitorStreaming:
    ...
    def __init__(self, logs_monitor_service, log_file_path: str, test_logger=None):
        ...
        self.logger = test_logger or get_logger("framework.handler.log_monitor_streaming")
    ...
    def _wait_for_matching_entries(self, stream_id: str, criteria: EventCriteria) -> Optional[List[LogEntry]]:
        ...
        elapsed = time.time() - start_time
        self.logger.warning(
            f"Timeout after {elapsed:.2f}s - found {len(matching_entries) if 'matching_entries' in locals() else 0} entries (needed {criteria.min_entries_required})")
        return None
