...
def test_complete_tap_to_login(login_state, session_manager, recent_entries, log_service, test_config, test_logger):
    """
    ...
    # STEP 2: Perform login tap and verify using command
    test_logger.info("STEP 2: Performing login tap and verifying using command")
    ...
    test_logger.info("User verification using command passed")
    ...
    user_match = re.search(r'Log Out ([^…]+)', applescript_output)
    if user_match:
    ...
    test_logger.debug(f"Executing AppleScript command: {apple_menu_script}")
